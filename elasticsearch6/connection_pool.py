...
class ConnectionSelector(object):
    ...
    *currently* live connections to choose from.
    ...
    :class:`~elasticsearch6.Transport` as `hosts` param and the same that is
...
class ConnectionPool(object):
    """
    Container holding the :class:`~elasticsearch6.Connection` instances,
    managing the selection process (via a
    :class:`~elasticsearch6.ConnectionSelector`) and dead connections.

    It's only interactions are with the :class:`~elasticsearch6.Transport` class
    ...
    def __init__(self, connections, dead_timeout=60, timeout_cutoff=5,
        ...
        :arg connections: list of tuples containing the
            :class:`~elasticsearch6.Connection` instance and it's options
        ...
        :arg selector_class: :class:`~elasticsearch6.ConnectionSelector`
